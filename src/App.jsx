...
const resumenItem = it => {
  ...
  if (it.producto==="eslabonada") return `${it.metros||"?"}m² | ${it.ancho||"?"}×${it.alto||"?"}m | Ab:${it.abertura||"?"} | Cal:${it.calibre||"?"}`;
  if (it.producto==="pvc")        return `${it.metros||"?"}m² | ${it.ancho||"?"}×${it.alto||"?"}m | Ab:${it.abertura||"?"} | Cal:${it.calibre||"?"} | CalInt:${it.calibreInterno||"?"} | ${it.color||"?"}`;
  if (it.producto==="postes")     return `Cal:${it.calibre||"?"} | ${it.grosor||"?"}" | ${it.largo||"?"}m | ${it.cantidad||"?"} un`;
  if (it.producto==="tubos")      return `Cal:${it.calibre||"?"} | ${it.grosor||"?"}" | ${it.largo||"?"}m | ${it.cantidad||"?"} un`;
  return "";
};

const exportCSV = rows => {
  const hdr=["No.Orden","Cliente","Sede","Creado por","Estado Orden","Creado","Completado","Producto","Estado Item","Máquina","M²","Ancho","Alto","Abertura","Calibre","Cal.Interno","Color","Grosor","Largo","Cantidad"];
  const stat={queue:"En Cola",active:"En Produccion",completed:"Completada"};
  const data=[];
  rows.forEach(o=>{
    const items=normalizeItems(o);
    if(items.length===0){
      data.push([o.orden,o.cliente,o.sede,o.vendedoraName,stat[o.status]||o.status,fmtDate(o.timestamp),o.completedAt?fmtDate(o.completedAt):"—","—","—","—","—","—","—","—","—","—","—","—","—","—"]);
    } else {
      items.forEach((it,idx)=>data.push([
        idx===0?o.orden:"",idx===0?o.cliente:"",idx===0?o.sede:"",
        idx===0?o.vendedoraName:"",idx===0?(stat[o.status]||o.status):"",
        idx===0?fmtDate(o.timestamp):"",idx===0?(o.completedAt?fmtDate(o.completedAt):"—"):"",
        labelProducto(it.producto),stat[it.status]||it.status||"—",
        it.machineLabel||"—",it.metros||"—",it.ancho||"—",it.alto||"—",
        it.abertura||"—",it.calibre||"—",it.calibreInterno||"—",it.color||"—",
...
function Shell({user,onLogout,orders}){
  ...
  return(
    <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"system-ui,-apple-system,sans-serif"}}>
      ...
      {modal?.t==="assignOrder" &&<AssignOrderModal order={modal.order} allOrders={orders} machines={MACHINES} user={user} isG={isG} onClose={()=>setModal(null)} onAssign={assignItem}/>}
  ...
  return(
    <div>
      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <input style={{...inp,flex:1,minWidth:200}} placeholder="Buscar por No. Orden o cliente..." value={q} onChange={e=>setQ(e.target.value)}/>
        <button onClick={()=>exportCSV(fil)} style={btnG}>Exportar Excel</button>
      </div>
  ...
  return(
    <Modal title="Crear Nueva Orden" onClose={onClose} maxWidth={580}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        ...
        <div><label style={{fontSize:13,fontWeight:600,color:"#64748b",display:"block",marginBottom:5}}>Cliente *</label><input style={inp} value={cliente} onChange={e=>setCliente(e.target.value)}/></div>
      </div>
...
function AssignOrderModal({order,allOrders,machines,user,isG,onClose,onAssign}){
  ...
  const availableMachines=(itemIdx)=>{
    const sedesPermitidas=isG?["Centro","Santa Lucia"]:[user.sede];
  ...
  const submit=async()=>{
    if(Object.keys(sel).length===0){setErr("Selecciona al menos una máquina");return;}
    setLoading(true);
    // Asignar en secuencia
    for(const [idxStr,machineId] of Object.entries(sel)){
